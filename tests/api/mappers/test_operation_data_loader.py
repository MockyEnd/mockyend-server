			...
			"values": [
				{
					"object_value": {
						"class_name": "Person",
						"attributes": {"name": "Raphael", "age": 35},
					}
				},
				{
					"object_value": {
						"class_name": "Person",
						"attributes": {"name": "Marianne", "age": 30},
					}
				},
			],
		},
...
@pytest.fixture
def data_template_with_list_of_class_type(
	raw_list_data_template_with_class_type,
) -> DataTemplate:
...
def create_data_template_with_list_of_primitive_type(
	data_type: DataType, values: List
) -> DataTemplate:
	raw_data = {
		"data_type": "LIST",
		"list_value": {"data_type": data_type.value, "values": values},
	}
...
@pytest.mark.parametrize(
	...
	[
		...
		(
			DataType.DICT,
			[{"status": "OK", "count": 10}, {"status": "Not OK", "count": 0}],
		),
	],
)
def test_data_loader_with_list_of_primitive_values(data_type: DataType, values: List):
	# Given
	data_template = create_data_template_with_list_of_primitive_type(
		data_type=data_type, values=values
	)
		...
		"object_value": {
			...
			"attributes": {
				...
				"sender": {
					...
					"object_value": {
						"class_name": "Customer",
						"attributes": {"name": "Sender", "document": "123.456.789-01"},
					},
				},
				"recipient": {
					"data_type": DataType.CLASS,
		...
		"object_value": {
			...
			"attributes": {
				...
				"sender": {
					...
					"object_value": {"attributes": {"name": "Sender", "document": "123.456.789-01"}}
				},
			},
		},
	}
