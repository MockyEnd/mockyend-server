...
from app.domain.models.operation import Method
...
class DataLoader:
	...
	def compile(self, data_template: DataTemplate) -> object:
		return self.load(data_template=data_template)

	def load(self, data_template: DataTemplate) -> object:
	...
	def build_object(self, data_template: DataTemplate) -> object:
		...
		for attribute, value in attributes_values.items():
			if isinstance(value, dict):
				...
				except ClassNameNotProvidedError:
	...
	@staticmethod
	def _to_value(create_type_function, data_template: DataTemplate | Any) -> object:
...
class CreateOperationMapper:
	@staticmethod
	def mapping(request: CreateEndpointRequest) -> OperationCreate:
		response_type, response_content = None, None
		if response := request.response:
			response_type = response.data_type
			response_content = DataLoader().load(data_template=response)
